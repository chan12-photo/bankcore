...
- pessimistic lock 실험: `PESSIMISTIC_WRITE`로 account row를 직렬화해 두 번째 요청이 최신 잔액을 보도록 만들었습니다. 실제 internal transfer는 두 계좌를 낮은 account id부터 순차적으로 잠가 반대 방향 동시 이체의 deadlock 위험을 줄였습니다.
